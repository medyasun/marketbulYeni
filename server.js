...
/*     const capitalize = (s) => {
    ...
    for (let i = 1; i <= urlEL; i++) {
      ...
      for (let i = 0; i < aramaLength; i++) {
        ...
        else if(aramaLength-1==i){  aramaBirlestir+=jsonSonuclar.name.toLocaleLowerCase('tr-TR').includes(aramaSplit[i])}
...
var urlE2L=urlElems2.length;
  for (let i = 1; i <= urlE2L; i++) {
    ...
    for (let i = 0; i < aramaLength; i++) {
      ...
      else if(aramaLength-1==i){  aramaBirlestir2+=jsonSonuclar2.name.toLocaleLowerCase('tr-TR').includes(aramaSplit[i])}
